...
var createGame = function createGame() {
  var maindeckDiv = document.createElement('div');
  maindeckDiv.setAttribute('class', 'maindeckDiv');
  var player1CardsDiv = document.createElement('div');
  player1CardsDiv.setAttribute('class', 'player1CardsDiv'); // player 1 name

  var DivPlayer1Name = document.createElement('div');
  DivPlayer1Name.innerHTML = 'player 1'; // this is the card div for cards 

  var Divplayer1cards = document.createElement('div');
  Divplayer1cards.setAttribute('class', 'Divplayer1cards');
  player1CardsDiv.append(DivPlayer1Name, Divplayer1cards);
  var player2CardsDiv = document.createElement('div');
  player2CardsDiv.setAttribute('class', 'player2CardsDiv');
  var DivPlayer2Name = document.createElement('div');
  DivPlayer2Name.innerHTML = 'player 2'; // this is the card div for cards 

  var Divplayer2cards = document.createElement('div');
  Divplayer2cards.setAttribute('class', 'Divplayer2cards');
  player2CardsDiv.append(DivPlayer2Name, Divplayer2cards);
  mainDiv.append(player1CardsDiv, maindeckDiv, player2CardsDiv); // create deck or game state 

  axios__WEBPACK_IMPORTED_MODULE_0___default().get('/startGame').then(function (gameState) {
    currentGame = gameState.data;
    currentPlayer = currentGame.currentplayer;
    console.log(currentPlayer);
    console.log(currentGame.player1Hand);
    console.log(currentGame.player2Hand); // display player1 and player 2 cards 

    displayCards(currentGame.player1Hand, Divplayer1cards);
    displayCards(currentGame.player2Hand, Divplayer2cards); //display deck 

    displayCardDeck(currentGame.cardDeck, maindeckDiv);
  });
};

var displayCards = function displayCards(playerhand, playerDiv) {
  for (var i = 0; i < playerhand.length; i++) {
    var SingleCardDiv = document.createElement('div');
...
var cardRankArray = ['ace', '2', '3', '4', '5', '6', '7', '8', '9', '10', 'jack', 'queen', 'king'];

var displayCardDeck = function displayCardDeck(deck, deckDiv) {
  // left handle card deck count and card deck image
  var leftDeckDiv = document.createElement('div');
  leftDeckDiv.setAttribute('class', 'leftDeckDiv');
  var remaindingcardDiv = document.createElement('div');
  remaindingcardDiv.setAttribute('class', 'remaindingcardDiv');
  remaindingcardDiv.innerHTML = "remainding cards in deck = ".concat(deck.length);
  var numberofBookDiv = document.createElement('div');
  numberofBookDiv.setAttribute('class', 'numberofBookDiv');
  numberofBookDiv.innerHTML = "player 1 book = 0 </br>\n   player 2 book = 0";
  leftDeckDiv.append(remaindingcardDiv, numberofBookDiv); // middle handle go fish and buttons and show states 

  var middleDeckDiv = document.createElement('div');
  middleDeckDiv.setAttribute('class', 'middleDeckDiv');
  var selectDiv = document.createElement('div');
  var cardStateDiv = document.createElement('div');
  cardStateDiv.setAttribute('class', 'cardStateDiv');
  cardStateDiv.innerHTML = "current state = player 1 go first";
  var selectCardSelect = document.createElement('select');
  selectCardSelect.setAttribute('id', 'selectCardSelect');

  for (var i = 0; i < cardRankArray.length; i++) {
    var option = document.createElement('option');
    option.text = cardRankArray[i];
    selectCardSelect.options.add(option, i);
  }

  var selectCardButton = document.createElement('button');
  ...
  middleDeckDiv.append(selectDiv, cardStateDiv); // right handle rules

  var rightDeckDiv = document.createElement('div');
  rightDeckDiv.setAttribute('class', 'rightDeckDiv');
  var RuleHeader = document.createElement('label');
...
var GoFish = function GoFish() {
  var getSelect = document.getElementById('selectCardSelect');
  ...
  try {
    axios__WEBPACK_IMPORTED_MODULE_0___default().post("/game/goFish/".concat(currentGame.id), playerSelectedCard).then(function (response) {
      // need to update results 
      console.log(response);
      var data = response.data; // update player 1 hand 

      var player1cardDiv = document.querySelector('.Divplayer1cards');
      console.log(player1cardDiv);
      player1cardDiv.innerHTML = '';
      displayCards(data.player1Hand, player1cardDiv); // update player 2 hand

      var player2cardDiv = document.querySelector('.Divplayer2cards');
      player2cardDiv.innerHTML = '';
      displayCards(data.player2Hand, player2cardDiv); // update books 

      var numberofBookDiv = document.querySelector('.numberofBookDiv');
      numberofBookDiv.innerHTML = "player 1 book = ".concat(data.player1Book, " </br>\n        player 2 book = ").concat(data.player2Book); // update remainding cards 

      var remaindingcardDiv = document.querySelector('.remaindingcardDiv');
      remaindingcardDiv.innerHTML = "remainding cards in deck = ".concat(data.remanindingCards.length); // update current state 

      var cardStateDiv = document.querySelector('.cardStateDiv');

      if (data.winner === '') {
        cardStateDiv.innerHTML = "current state = player ".concat(data.currentplayer);
        currentPlayer = data.currentplayer;
      } else {
        cardStateDiv.innerHTML = "winner is  player ".concat(data.winner);
      }
    });
  } catch (ex) {
    console.log(ex);
...
_asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee2() {
  var ws, connectToServer, _connectToServer, getOrCreateCursorFor;

  return regeneratorRuntime.wrap(function _callee2$(_context2) {
    while (1) {
      switch (_context2.prev = _context2.next) {
        case 0:
          getOrCreateCursorFor = function _getOrCreateCursorFor(messageBody) {
            var sender = messageBody.sender;
            var existing = document.querySelector("[data-sender='".concat(sender, "']"));

            if (existing) {
              return existing;
            }

            var template = document.getElementById('cursor');
            var cursor = template.content.firstElementChild.cloneNode(true);
            var svgPath = cursor.getElementsByTagName('path')[0];
            cursor.setAttribute("data-sender", sender);
            svgPath.setAttribute('fill', "hsl(".concat(messageBody.color, ", 50%, 50%)"));
            document.body.appendChild(cursor);
            return cursor;
          };

          _connectToServer = function _connectToServer3() {
            _connectToServer = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee() {
              var ws;
              return regeneratorRuntime.wrap(function _callee$(_context) {
                while (1) {
                  switch (_context.prev = _context.next) {
                    case 0:
                      ws = new SockJS('http://localhost:7071/ws');
          ...
          connectToServer = function _connectToServer2() {
            return _connectToServer.apply(this, arguments);
          };
        ...
        case 5:
          ...
          ws.onmessage = function (webSocketMessage) {
            ...
            var cursor = getOrCreateCursorFor(messageBody);
            cursor.style.transform = "translate(".concat(messageBody.x, "px, ").concat(messageBody.y, "px)");
          };
